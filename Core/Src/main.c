...
int main(void)
{
 ...
 /*HAL_ADC_Start(&hadc1);
      ...
      int len = sprintf(buffer, "%lu", AD_RES); // Convert ADC value to string
...
{  if(htim->Instance==TIM3){
      ...
      /* HAL_ADC_Start(&hadc1);
       ...
       uint32_t AD_RES = HAL_ADC_GetValue(&hadc1);
       char buffer[16];
       memset(buffer, 0, sizeof(buffer)); // Clear the buffer before printing a new value
       int len = snprintf(buffer, sizeof(buffer), "%d\n", AD_RES);
       HAL_UART_Transmit(&huart1, (uint8_t*) buffer, len, HAL_MAX_DELAY);*/

// Start ADC conversion for ADC1 and ADC2 simultaneously
      HAL_ADC_Start(&hadc1);
       HAL_ADC_Start(&hadc2);

       // Wait for conversion to complete for both ADCs
       while (HAL_ADC_PollForConversion(&hadc1, 1) != HAL_OK);
       while (HAL_ADC_PollForConversion(&hadc2, 1) != HAL_OK);

       // Read the converted values from both ADCs
